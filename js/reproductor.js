...
$(function(){
	...
	$('#btnPausar').on('click',function(){
		...
		if(objReproductor.paused){
			$('#btnPausar').css("background-image", "url(img/pausar.png)"); 
			...
			$('#btnPausar').css("background-image", "url(img/reproducir.png)"); 
	...
	$(document).ready(function (){
		//Iniciar reproduciendo el audio.
		$('#btnReproducir').trigger('click');	
	})
	
	//actualizar el tiempo transcurrido y la barra de progreso
	$(objReproductor).on('timeupdate',function(){
		//Pausar el audio apenas comienza.
		var pause=true;
		if (objReproductor.currentTime>0 && objReproductor.currentTime<0.4 && pause){ 
			$('#btnPausar').trigger('click');
			pause=false;
			}
